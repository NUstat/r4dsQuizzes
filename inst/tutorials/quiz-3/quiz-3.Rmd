...
if(sample_data == 1){
  ...
  q1[[3]] <- HTML("Create a variable called <code>cents</code> which is derived from the variable <code>price</code> ie: For a <code>price</code> of 13.20 the <code>cents</code> would be 20.")
} else{
...
if(sample_data == 1){
  ...
  q2[[3]] <- HTML("Use <code>mutate()</code> in conjunction with <code>if_else()</code> to create a new variable called <code>high_alt</code> that is TRUE if the <code>alt</code> is greater than 1500 and has a value of FALSE, otherwise.")
} else if(sample_data == 2){
...
if(sample_data == 1){
  ...
  q4[[2]] <- HTML("Use a function from the <code>tidyr</code> (preferred) or <code>stringr</code> package to separate <code>id</code> into a column called <code>year</code> and <code>order</code>. ie: the first observation '2015-000001' will have a <code>year</code> 2015 and <code>order</code> 000001 (or 1).")
